...
function History() {
  ...
  const openNotes = async (notedId) => {
    setLoading(true);
    setActiveNoteId(notedId);
    try {
      const res = await axios.get(serverUrl + `/api/notes/${notedId}`, {
        withCredentials: true,
      });
                ...
                <ul className="space-y-3">
                  {topics.map((t, i) => (
                    <li
                      ...
                      className={`cursor-pointer rounded-xl p-3 border transition-all ${activeNoteId === t._id ? "bg-indigo-500/30 border-indigo-400 shadow-[0_0px_0px_rgba(99,102,241,0.6)]" : "bg-white/5 border-white/10 hover:bg-white/10"
